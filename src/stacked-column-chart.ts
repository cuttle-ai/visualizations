...
/*
 * This file contains the definition of stacked column chart visualization
 */

/**
 * StackedColumnChartVisualization will draw a stacked column chart for the given visualization data
 *
 * @example let sccv = new ColumnChartVisualization(id, queryResult); sccv.render();
 */
export class StackedColumnChartVisualization implements Viz {
  /**
   * htmlElementId is the id of the html element on which the stacked column chart has to be rendered
   */
  htmlElementId: string;
  /**
   * queryResult on which the stacked column chart visualization is created
   */
  queryResult: QueryResult | null = null;
  /**
   * htmlElement holds the html element instance on which the stacked column chart is drawn
   */
  ...
  render() {
    ...
    this.htmlElement.innerHTML = '<div id="' + this.htmlElementId + '-cuttle-stacked-column-chart"></div>';
    const xaxis = this.getXaxisValues();
    const yAxes = this.getYAxesValues();
    Highcharts.chart(this.htmlElementId + '-cuttle-stacked-column-chart', {
      chart: {
        type: 'column',
      },
      title: {
