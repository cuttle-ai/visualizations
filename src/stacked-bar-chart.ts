...
/**
 * StackedBarChartVisualization will draw a stacked bar chart for the given visualization data
 *
 * @example let sbcv = new StackedBarChartVisualization(id, queryResult); sbcv.render();
 */
export class StackedBarChartVisualization implements Viz {
  /**
   * htmlElementId is the id of the html element on which the stacked bar chart has to be rendered
   */
  htmlElementId: string;
  /**
   * queryResult on which the stacked bar chart visualization is created
   */
  queryResult: QueryResult | null = null;
  /**
   * htmlElement holds the html element instance on which the stacked bar chart is drawn
   */
  htmlElement: HTMLElement | null = null;

  /**
   * type of the visuzaltion is stacked bar chart
   */
  type: VizType = VizType.StackedBarChart;
  ...
  render() {
    ...
    this.htmlElement.innerHTML = '<div id="' + this.htmlElementId + '-cuttle-stacked-bar-chart"></div>';
    const yaxis = this.getYaxisValues();
    const xAxes = this.getXAxesValues();
    Highcharts.chart(this.htmlElementId + '-cuttle-stacked-bar-chart', {
      chart: {
        type: 'bar',
      },
      title: {
